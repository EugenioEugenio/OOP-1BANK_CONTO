...
class BusinessAccount(Account):
    ...
    def withdraw(self, amount):
        ...
        if amount > 0:
            ...
            if amount <= self.__balance + self.overdraft_limit:
                ...
                if self.__balance < 0:
                    print(f"Withdrawn using overdraft: {amount}. Текущий баланс: {self.__balance}")
                else:
                    print(f"Снято {amount} в пределах баланса. Текущий баланс: {self.__balance}")
            else:
                print(f"Exceeds overdraft limit. Попытка снять {amount}, доступно {self.__balance + self.overdraft_limit}.")
        else:
            print("Сумма снятия должна быть положительной.")

    def get_balance(self):
        Account.get_balance(self) #Возвращает текущий баланс счета."""
    ...
    def __str__(self):
        ...
        return f"BusinessAccount Holder: {self.account_holder}, Баланс : {self.__balance}, Overdraft Limit: {self.overdraft_limit}"
